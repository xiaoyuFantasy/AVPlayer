...
void CFileDialogEx::ShowOpenFileDlg(FileDialogCallback2 file_dialog_callback2)
{
...
void CFileDialogEx::ShowSaveFileDlg(FileDialogCallback2 file_dialog_callback2)
{
	file_dialog_callback2_ = file_dialog_callback2;
...
void CFileDialogEx::SyncShowModal()
{
	if (file_dialog_type_ == FDT_OpenFile)
	{
		...
		if ((m_stOFN.Flags & OFN_ALLOWMULTISELECT) > 0)
		{
			...
			if (file_name.size() == 0)
			{
				file_dialog_callback2_(ret, file_directory);
			}
			else
			{
				while (file_name.size() > 0)
				{
					...
					{
						file_dialog_callback2_(ret, file_path);
					}
		...
		{
			file_dialog_callback2_(ret, GetPathName());
		}
	}
